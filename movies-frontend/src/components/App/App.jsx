...
function App() {
  const location = useLocation(); // данные url
  const navigate = useNavigate(); // перемещения по url
  ...
  function handleSearchFilms(value, checkbox) {
    ...
    if (!allMovieslist.length) {
      setIsBlockingButton(true); // отключаем кнопку отправки формы

      setIsLoadingMovies(true);
      api
        .getMovies()
        .then((movies) => {
          setIsBlockingButton(false); // включаем кнопку отправки формы
